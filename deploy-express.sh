#!/usr/bin/env bash
#
# Express 版を Raspberry Pi へ置く。旧 Flask 版には触れない。
#
#   PI_PASS='<パスワード>' ./deploy-express.sh
#
# 8080 は Flask が握ったまま。Express は EX_PORT で、BASE_PATH の下に立つ。
# データベースは Flask が使っているものをそのまま見る。WAL なので 2 つの
# プロセスが同時に開けて、片方の書き込みがもう片方から見える。
#
# 8080 を 2 つで分け合う前段の設置は switch-front.sh が受け持つ。ここでは行わない。
...
LIVE_DB="${LIVE_DB:-/home/pi/furo-gohan/data/furo-gohan.db}"
...
echo "-- データベースの控えを取る --"
test -f "$LIVE_DB" || { echo "データベースが無い: $LIVE_DB" >&2; exit 1; }
...
[Unit]
Description=kominka-reserver (Express 版・並走)
...
echo "   Flask は $(systemctl is-active furo-gohan) のまま"
REMOTE
...
echo
echo "Express: http://$PI_HOST:$EX_PORT$BASE_PATH/bath"
echo "Flask  : http://$PI_HOST:8080/bath  (触っていない)"
